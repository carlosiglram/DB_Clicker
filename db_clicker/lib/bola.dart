...
class Principal extends State<Bola> {
  ...
  Widget build(BuildContext context) {
    if (!login) {
      login = true;
      final datosPartida = ModalRoute.of(context)!.settings.arguments as DatosPartida;
      clicks = datosPartida.clicks;
      clicksbonus = datosPartida.clicksbonus;
      contador = datosPartida.contador;
      mejoraBola = datosPartida.mejoraBola;
      mejoraGoku = datosPartida.mejoraGoku;
      mejoraVegeta = datosPartida.mejoraVegeta;
      mejoraGohan = datosPartida.mejoraGohan;
      mejoraTrunks = datosPartida.mejoraTrunks;
      mejoraCelula = datosPartida.mejoraCelula;
      mejoraFreezer = datosPartida.mejoraFreezer;
      mejoraBuu = datosPartida.mejoraBuu;
      gokus = datosPartida.gokus;
      vegetas = datosPartida.vegetas;
      gohans = datosPartida.gohans;
      trunks = datosPartida.trunks;
      freezers = datosPartida.freezers;
      celulas = datosPartida.celulas;
      buus = datosPartida.buus;
    }
      ...
      body: Container(
        ...
        child: Center(
          ...
          children: [
            ...
            Visibility(
                child: Container(
                    padding: EdgeInsets.only(left: 295, top: 66),
                    child: InkWell(
            ...
            Visibility(
                child: Container(
                    padding: EdgeInsets.only(left: 295, top: 32),
                    child: InkWell(
                        child: tiempo == 1000 ? Image.asset("assets/icons/save.png", height: 25) : Image.asset("assets/icons/save_b.png", height: 25),
                        onTap: () => guardarPartidaJugador(Login2.nombre)))),
          ],
        )),
      ),
...
//Funciones de moverse entre ventanas
  ...
  Widget texto1() {
    return Text(
      "" + (produccion).toString() + " zenis /s.",
      style: TextStyle(fontFamily: 'JetBrains', fontSize: 14, fontWeight: FontWeight.bold, color: Colors.yellow.shade600),
  ...
  Widget texto2() {
    return Text(
      "Total: " + totalPj(produccion, workers) + " zenis /s.",
      style: TextStyle(fontFamily: 'JetBrains', fontSize: 14, fontWeight: FontWeight.bold, color: Colors.yellow.shade600),
      textAlign: TextAlign.center,
    );
  }
  ...
  void mensajeGuardar(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(
        "PARTIDA GUARDADA",
        style: TextStyle(color: Colors.black),
  ...
  guardarPartidaJugador(String nombre) async {
    ConectionUser con = ConectionUser();

    bool comprobarGuardado = await con.guardarPartida(nombre);

    if (comprobarGuardado) {
      mensajeGuardar(context);
    }
  }
}
